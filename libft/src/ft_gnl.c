...
/*   Updated: 2016/09/15 15:56:19 by auverneu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** You can adjust the buff size with the macro FT_GNL_BUFF.
...
static t_fd		*create_fd(t_fd *file, int fd)
{
	...
	if (new == NULL)
		return (NULL);
	new->strt = ft_strnew(FT_GNL_BUFF + 1);
	new->buff = new->strt;
	new->fd = fd;
	if (file)
	{
...
static int		readnext(t_fd *file)
{
	...
	ft_bzero(file->strt, FT_GNL_BUFF);
	ret = read(file->fd, file->strt, FT_GNL_BUFF);
	file->buff = file->strt;
	return (ret);
}
...
int				ft_gnl(int const fd, char **line)
{
	...
	if (FT_GNL_BUFF <= 0 || file == NULL || !line)
		return (-1);
	*line = ft_strnew(1);
	ret = 1;
	if (*(file->buff) == '\0')
